...
<div class="flex items-center justify-between mb-6">
    <div>
        <div class="text-xl font-bold text-[#1a3a5c] tracking-tight">Modifier la séance</div>
        <div class="text-sm text-[#5a8aaa] mt-0.5">Mettre à jour les informations de la séance</div>
    </div>
    <a href="{{ route('formateur.seances.index') }}"
...
<div class="bg-white border border-[#cde4f0] rounded-xl shadow-sm overflow-hidden">
    ...
    {{-- Card Body --}}
    <div class="p-6">
        <form action="{{ route('formateur.seances.update', $seance) }}" method="POST">
            @csrf @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input type="date" name="date_seance"
                       value="{{ old('date_seance', $seance->date_seance) }}"
            ...
            <div class="mb-4">
                ...
                <select name="group_id"
                    ...
                    @foreach($groupes as $groupe)
                        <option value="{{ $groupe->id }}" {{ old('group_id', $seance->group_id) == $groupe->id ? 'selected' : '' }}>
                            {{ $groupe->nom }}
                        </option>
                    @endforeach
                </select>
                ...
                <div>
                    ...
                    <select name="type"
                        ...
                        @foreach(['cours' => 'Cours', 'td' => 'TD', 'tp' => 'TP', 'controle' => 'Contrôle', 'examen' => 'Examen', 'rattrapage' => 'Rattrapage'] as $val => $label)
                            <option value="{{ $val }}" {{ old('type', $seance->type) == $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="status"
                        ...
                        @foreach(['programmee' => 'Programmée', 'terminee' => 'Terminée', 'annulee' => 'Annulée', 'reportee' => 'Reportée'] as $val => $label)
                            <option value="{{ $val }}" {{ old('status', $seance->status) == $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="description" rows="3"
                          class="w-full px-3 py-2.5 rounded-lg border-[1.5px] border-slate-200 bg-slate-50 text-[13.5px] text-slate-800 font-['Inter'] outline-none focus:border-[#4fa3d1] focus:bg-white focus:shadow-[0_0_0_3px_rgba(79,163,209,0.12)] transition-all duration-150 resize-y">{{ old('description', $seance->description) }}</textarea>
            </div>
