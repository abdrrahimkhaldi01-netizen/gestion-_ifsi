...
<div class="max-w-2xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <div class="text-xl font-bold text-[#1a3a5c] tracking-tight">Nouveau module</div>
            <div class="text-sm text-[#5a8aaa] mt-0.5">Ajouter un module de formation</div>
        </div>
    ...
    <div class="bg-white border border-[#cde4f0] rounded-xl shadow-sm overflow-hidden">
        ...
        <div class="p-6">
            <form action="{{ route('directeur.modules.store') }}" method="POST" id="moduleForm">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="titre" value="{{ old('titre') }}" placeholder="Titre du module"
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" rows="3" placeholder="Description du module..."
                              class="w-full px-3 py-2.5 rounded-lg border-[1.5px] border-slate-200 bg-slate-50 text-[13.5px] text-slate-800 outline-none focus:border-[#4fa3d1] focus:bg-white transition-all resize-y @error('description') border-red-400 bg-red-50 @enderror">{{ old('description') }}</textarea>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="duree" value="{{ old('duree') }}" placeholder="Ex: 40"
                           min="1" step="1"
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="nombre_cc" value="{{ old('nombre_cc') }}"
                        ...
                        <select id="filiere_filter"
                            ...
                            @foreach($filieres as $filiere)
                                <option value="{{ $filiere->id }}"
                                    {{ old('_filiere_filter') == $filiere->id ? 'selected' : '' }}>
                                    {{ $filiere->nom }}
                                </option>
                            @endforeach
                        </select>
...
<div id="old-unite-id" style="display:none">{{ json_encode(old('unite_id')) }}</div>
<div id="old-groupes" style="display:none">{{ json_encode(old('groupes', [])) }}</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const oldUniteId     = JSON.parse(document.getElementById('old-unite-id').textContent);
    const oldGroupes     = JSON.parse(document.getElementById('old-groupes').textContent);
    ...
    function buildUnites(filiereId) {
        ...
        filtered.forEach(u => {
            ...
            if (oldUniteId && String(u.id) === String(oldUniteId)) opt.selected = true;
    ...
    function buildGroupes(filiereId) {
        ...
        filtered.forEach(g => {
            ...
            const checked = oldGroupes.map(String).includes(String(g.id)) ? 'checked' : '';
            label.innerHTML = `
    ...
    // Restore old() values
    if (oldUniteId) {
        const oldUnite = allUnites.find(u => String(u.id) === String(oldUniteId));
        if (oldUnite && oldUnite.filiere_id) {
            filiereSelect.value = String(oldUnite.filiere_id);
            buildUnites(oldUnite.filiere_id);
            buildGroupes(oldUnite.filiere_id);
        }
    } else {
        buildUnites('');
        buildGroupes('');
