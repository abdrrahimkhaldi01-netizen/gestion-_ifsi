...
<div class="flex items-center justify-between mb-6">
    <div>
        <div class="text-xl font-bold text-[#1a3a5c] tracking-tight">Nouveau formateur</div>
        <div class="text-sm text-[#5a8aaa] mt-0.5">Ajouter un formateur ou encadrant</div>
    </div>
...
<div class="bg-white border border-[#cde4f0] rounded-xl shadow-sm overflow-hidden max-w-2xl">
    ...
    {{-- Card Body --}}
    <div class="p-6">
        <form action="{{ route('directeur.formateurs.store') }}" method="POST">
            @csrf
            ...
            {{-- Email --}}
            <div class="mb-4">
                <label class="block text-[11px] font-bold text-[#5a8aaa] uppercase tracking-wider mb-1.5">Email</label>
                <input type="email" name="email" value="{{ old('email') }}" placeholder="email@example.com"
                       class="w-full px-3 py-2.5 rounded-lg border-[1.5px] border-slate-200 bg-slate-50 text-[13.5px] text-slate-800 outline-none focus:border-[#4fa3d1] focus:bg-white focus:shadow-[0_0_0_3px_rgba(79,163,209,0.12)] transition-all duration-150 @error('email') border-red-400 @enderror">
                @error('email')
                    <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            {{-- Mot de passe --}}
            <div class="mb-4">
                <label class="block text-[11px] font-bold text-[#5a8aaa] uppercase tracking-wider mb-1.5">Mot de passe</label>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" name="telephone" value="{{ old('telephone') }}" placeholder="0600000000"
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                ...
                <div>
                    ...
                    <input type="text" name="specialite" value="{{ old('specialite') }}" placeholder="Ex: Informatique"
